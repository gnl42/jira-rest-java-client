...
public class IssueJsonParser implements JsonObjectParser<Issue> {
    ...
    public Issue parse(final JSONObject issueJson) throws JSONException {
        ...
        final OffsetDateTime creationDate = JsonParseUtil.parseDateTime(getFieldStringUnisex(issueJson, CREATED_FIELD.id));
        final OffsetDateTime updateDate = JsonParseUtil.parseDateTime(getFieldStringUnisex(issueJson, UPDATED_FIELD.id));

        final String dueDateString = getOptionalFieldStringUnisex(issueJson, DUE_DATE_FIELD.id);
        final OffsetDateTime dueDate = dueDateString == null ? null : JsonParseUtil.parseDateTimeOrDate(dueDateString);
        ...
        final BasicIssue parent = getOptionalNestedField(issueJson, IssueFieldId.PARENT_FIELD.id, basicIssueJsonParser);
        return new Issue(summary, selfUri, basicIssue.getKey(), basicIssue.getId(), project, issueType, status,
                ...
                votes, worklogs, watchers, expandos, subtasks, changelog, operations, labels, parent);
    }
